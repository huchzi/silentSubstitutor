...
server <- shiny::shinyServer(function(input, output, session) {
  ...
  onRestored(function(state) {
    ...
    lapply(1:5, function(x) {
      updateSliderInput(session, paste0("luminance_primary_", x),
  ...
  led_contrasts_percent <- reactive({
    100 * find_LED_contrasts(
      photoreceptor_contrasts_percent() / 100,
      A_matrix()
    )
  })
  ...
  led_limits <- reactive({
    data.frame(
      LED = factor(globalVars$primary_names),
  ...
  led_luminances_over_time <- reactive({
    led_out <- data.frame(
      led = names(led_contrasts_percent()),
  ...
  observeEvent(input$upload_spectra, {
    ...
    new_spectra_choices(
      c(
        new_spectra_choices()[!grepl("^CUSTOM:", new_spectra_choices())],
  ...
  output$luminance_primary_5 <- renderUI({
    ...
    luminance_slider(5, globalVars$primary_names[5],
      input$upper5 / 2,
      min = input$lower5, max = input$upper5, step = input$step5
    )
  })
  ...
  output$led_contrasts <- renderTable(
    {
      ...
      for (i in 1:globalVars$n_primaries) {
        ...
        if (abs(as.numeric(tab_out$MichelsonContrast[i])) > 100.0001) {
          tab_out$MichelsonContrast[i] <- paste0(
            "<div style='color: white; background-color: red;'>",
    ...
    striped = TRUE,
    align = "lrrrrrr",
    caption = "Primary settings for achieving silent substitution",
    digits = 3,
